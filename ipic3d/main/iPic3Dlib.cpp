...
int c_Solver::Init(int argc, char **argv) {
  ...
  GPIUtil::init(grid->getNXC(), grid->getNYC());
  if(GPIUtil::getGPINprocs() != vct->getNprocs() || GPIUtil::getGPIRank() != myrank)
  {
    cout << "GPIUtil::getGPINprocs() != vct->getNprocs() || GPIUtil::getGPIRank() != myrank" << endl;
    MPIdata::instance().finalize_mpi();
    return (-1);
  }
