...
void VCtopology3D::setup_vctopology(MPI_Comm old_comm) {
  ...
  if (CART_COMM != MPI_COMM_NULL) {
    ...
    int coordXleft   = coordinates[0]-1;
    int coordXright = coordinates[0]+1;
    int coordYleft   = coordinates[1]-1;
    int coordYright = coordinates[1]+1;
    
    if( coordXleft==-1&& periods[0]==1){
      coordXleft = dims[0]-1;
    }
    if( coordXright==dims[0] && periods[0]==1){
      coordXright = 0;
    }
    if( coordYleft==-1 && periods[1]==1){
      coordYleft = dims[1]-1;
    }
    if( coordYright==dims[1] && periods[1]==1){
      coordYright = 0;
    }

    if( coordXleft>-1 && coordXleft<dims[0] && coordYleft>-1 && coordYleft<dims[1]){
      int newcoords[3] = {coordXleft, coordYleft, coordinates[2]};
      MPI_Cart_rank(CART_COMM, newcoords, &xleftyleft_neighbor);
    }else{
      xleftyleft_neighbor = MPI_PROC_NULL;
    }

    if( coordXleft>-1 && coordXleft<dims[0] && coordYright>-1 && coordYright<dims[1]){
      int newcoords[3] = {coordXleft, coordYright, coordinates[2]};
      MPI_Cart_rank(CART_COMM, newcoords, &xleftyright_neighbor);
    }else{
      xleftyright_neighbor= MPI_PROC_NULL;
    }

    if( coordXright>-1 && coordXright<dims[0] && coordYleft>-1 && coordYleft<dims[1]){
      int newcoords[3] = {coordXright, coordYleft, coordinates[2]};
      MPI_Cart_rank(CART_COMM, newcoords, &xrightyleft_neighbor );
    }else{
      xrightyleft_neighbor = MPI_PROC_NULL;
    }

    if( coordXright>-1 && coordXright<dims[0] && coordYright>-1 && coordYright<dims[1]){
      int newcoords[3] = {coordXright, coordYright, coordinates[2]};
      MPI_Cart_rank(CART_COMM, newcoords, &xrightyright_neighbor);
    }else{
