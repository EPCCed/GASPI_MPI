  ...
  const double *b, int m, int max_iter, double tol, Field * field)
{
  ...
  double *local_result = new double[m + 3];
  ...
  for (itr = 0; itr < stop_itr; itr++)
  {
	...
	memcpy(local_result, y, sizeof(double) * (k + 2));
	SUCCESS_OR_DIE(gaspi_allreduce(local_result, y, (k + 2), GASPI_OP_SUM, GASPI_TYPE_DOUBLE, GASPI_GROUP_ALL, GASPI_BLOCK));
      }
      for (int j = 0; j <= k; j++) {
        H[j][k] = y[j];
