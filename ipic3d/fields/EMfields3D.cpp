...
void EMfields3D::MaxwellImage(double *im, double* vector)
{
    ...
    if(vct->getXright_neighbor()!= MPI_PROC_NULL || vct->getXleft_neighbor() != MPI_PROC_NULL || vct->getYright_neighbor()!= MPI_PROC_NULL || vct->getYleft_neighbor() != MPI_PROC_NULL)
    {
      double ***vectorTmp=divC.fetch_arr3();
      
      const gaspi_segment_id_t segment_id_send = GPIUtil::getSegIdSend();
      const gaspi_segment_id_t segment_id_recv = GPIUtil::getSegIdRecv();
      const gaspi_queue_id_t queue_id_send = GPIUtil::getQueueIdSend();
      const gaspi_queue_id_t queue_id_recv = GPIUtil::getQueueIdRecv();
      double* segment_ptr_send = (double*) GPIUtil::getSegPtrSend();
      double* segment_ptr_recv = (double*) GPIUtil::getSegPtrRecv();
      const gaspi_offset_t xOffset  = (nyc - 2) * sizeof(double);
      const gaspi_offset_t yOffset  = (nxc - 2) * sizeof(double);
      const gaspi_size_t xReadsize = xOffset;
      const gaspi_size_t yReadsize = yOffset;
      const gaspi_size_t cornerSize = sizeof(double);
      const gaspi_notification_t send_notification_value = 2;
      int   tot_recv_count = 0;
      const int myrank = vct->getCartesian_rank();

      if(vct->getXleft_neighbor() != MPI_PROC_NULL)
      {
	if(vct->getXleft_neighbor() != myrank)
	{
	  int curr_pos = 0;
	  for (int j = 1; j <= (nyc-2); j++)
	    *(segment_ptr_send + curr_pos++) = vectorTmp[1][j][1];
	  SUCCESS_OR_DIE(gaspi_write_notify(segment_id_send, 0, vct->getXleft_neighbor(), segment_id_recv, xOffset, xReadsize, (gaspi_notification_id_t) 2,send_notification_value, queue_id_send, GASPI_TEST));
	  tot_recv_count++;
	}
	else
	{
	  for (int j = 1; j <= (nyc-2); j++)
	    vectorTmp[nxc-1][j][1] = vectorTmp[1][j][1];
	}
        }
        
      if(vct->getXright_neighbor() != MPI_PROC_NULL)
      {
	if(vct->getXright_neighbor() != myrank)
	{
	  int curr_pos = 0;
	  for (int j = 1; j <= (nyc-2); j++)
	    *(segment_ptr_send + xOffset + curr_pos++) = vectorTmp[nxc-2][j][1];
	  SUCCESS_OR_DIE(gaspi_write_notify(segment_id_send, xOffset, vct->getXright_neighbor(), segment_id_recv, 0, xReadsize, (gaspi_notification_id_t) 1, send_notification_value, queue_id_send, GASPI_TEST));
	  tot_recv_count++;
	}
	else
	{
	  for (int j = 1; j <= (nyc-2); j++)
	    vectorTmp[0][j][1] = vectorTmp[nxc-2][j][1];
	}
      }
        
      if(vct->getYleft_neighbor() != MPI_PROC_NULL)
      {
	if(vct->getYleft_neighbor() !=myrank)
	{
	  int curr_pos = 0;
	  for (int i = 1; i <= (nxc-2); i++)
	    *(segment_ptr_send + xOffset*2 + curr_pos++) = vectorTmp[i][1][1];
	  SUCCESS_OR_DIE(gaspi_write_notify(segment_id_send, xOffset * 2, vct->getYleft_neighbor(), segment_id_recv, xOffset * 2 + yOffset, yReadsize, (gaspi_notification_id_t) 4, send_notification_value, queue_id_send, GASPI_TEST));
	  tot_recv_count++;
	}
	else
	{
	  for (int i = 1; i <= (nxc-2); i++)
	    vectorTmp[i][nyc-1][1] = vectorTmp[i][1][1];
	}
      }
	...
	if(vct->getYleft_neighbor() !=myrank)
	{
	  int curr_pos = 0;
	  for (int i = 1; i <= (nxc-2); i++)
	    *(segment_ptr_send + xOffset*2 + yOffset + curr_pos++) = vectorTmp[i][nyc-2][1];
	  SUCCESS_OR_DIE(gaspi_write_notify(segment_id_send, xOffset * 2 + yOffset, vct->getYright_neighbor(), segment_id_recv, xOffset * 2, yReadsize, (gaspi_notification_id_t) 3, send_notification_value, queue_id_send, GASPI_TEST));
	  tot_recv_count++;
	}
	else
	{
	  for (int i = 1; i <= (nxc-2); i++)
	    vectorTmp[i][0][1] = vectorTmp[i][nyc-2][1];
	}
      }
      
      if(vct->getXleftYleft_neighbor() != MPI_PROC_NULL)
      {
	if(vct->getXleftYleft_neighbor() != myrank)
	{
	  *(segment_ptr_send + xOffset*2 + yOffset*2 ) = vectorTmp[1][1][1];
	  SUCCESS_OR_DIE(gaspi_write_notify(segment_id_send, xOffset * 2 + yOffset * 2, vct->getXleftYleft_neighbor(), segment_id_recv, xOffset * 2 + yOffset * 2 + 3, cornerSize, (gaspi_notification_id_t) 8, send_notification_value, queue_id_send, GASPI_TEST));
	  tot_recv_count++;
	}
	else
	  vectorTmp[nxc-1][nyc-1][1] = vectorTmp[1][1][1];
      }
        
      if(vct->getXleftYright_neighbor() != MPI_PROC_NULL)
      {
	if(vct->getXleftYright_neighbor() != myrank)
	{
	  *(segment_ptr_send + xOffset*2 + yOffset*2 + 1) = vectorTmp[1][nyc-2][1];
	  SUCCESS_OR_DIE(gaspi_write_notify(segment_id_send, xOffset * 2 + yOffset * 2 + 1, vct->getXleftYright_neighbor(), segment_id_recv, xOffset * 2 + yOffset * 2 + 2, cornerSize, (gaspi_notification_id_t) 7, send_notification_value, queue_id_send, GASPI_TEST));
	  tot_recv_count++;
	}
	else
	  vectorTmp[nxc-1][0][1] = vectorTmp[1][nyc-2][1];
      }
        
      if(vct->getXrightYleft_neighbor() != MPI_PROC_NULL)
      {
	if(vct->getXrightYleft_neighbor() != myrank)
	{
	  *(segment_ptr_send + xOffset*2 + yOffset*2 + 2) = vectorTmp[nxc-2][1][1];
	  SUCCESS_OR_DIE(gaspi_write_notify(segment_id_send, xOffset * 2 + yOffset * 2 + 2, vct->getXrightYleft_neighbor(), segment_id_recv, xOffset * 2 + yOffset * 2 + 1, cornerSize, (gaspi_notification_id_t) 6, send_notification_value, queue_id_send, GASPI_TEST));
	  tot_recv_count++;
	}
	else
	  vectorTmp[0][nyc-1][1] = vectorTmp[nxc-2][1][1];
      }
        
      if(vct->getXrightYright_neighbor() != MPI_PROC_NULL)
      {
	if(vct->getXrightYright_neighbor() != myrank)
	{
	  *(segment_ptr_send + xOffset*2 + yOffset*2 + 3) = vectorTmp[nxc-2][nyc-2][1];
	  SUCCESS_OR_DIE(gaspi_write_notify(segment_id_send, xOffset * 2 + yOffset * 2 + 3,vct->getXrightYright_neighbor(), segment_id_recv, xOffset * 2 + yOffset * 2, cornerSize, (gaspi_notification_id_t) 5, send_notification_value, queue_id_send, GASPI_TEST));
	  tot_recv_count++;
	}
	else
	  vectorTmp[0][0][1] = vectorTmp[nxc-2][nyc-2][1];
      }

      if(tot_recv_count>0)
      {
	SUCCESS_OR_DIE(gaspi_wait(queue_id_send,GASPI_BLOCK));
	gaspi_notification_id_t recv_notification_id;
	gaspi_notification_t recv_notification_val;

	if(vct->getXleft_neighbor() != MPI_PROC_NULL && vct->getXleft_neighbor() != myrank)
	{
	  SUCCESS_OR_DIE(gaspi_notify_waitsome(segment_id_recv, (gaspi_notification_id_t)1, 1, &recv_notification_id, GASPI_BLOCK));
	  SUCCESS_OR_DIE(gaspi_notify_reset(segment_id_recv, recv_notification_id, &recv_notification_val));
	  assert(recv_notification_val == send_notification_value);
	  tot_recv_count--;
	  int curr_pos = 0;
	  for (int j = 1; j <= (nyc-2); j++)
	    vectorTmp[0][j][1] = *(segment_ptr_send + curr_pos++);
	}
            
	if(vct->getXright_neighbor() != MPI_PROC_NULL && vct->getXright_neighbor() != myrank)
	{
	  SUCCESS_OR_DIE(gaspi_notify_waitsome(segment_id_recv, (gaspi_notification_id_t)2, 1, &recv_notification_id, GASPI_BLOCK));
	  SUCCESS_OR_DIE(gaspi_notify_reset(segment_id_recv, recv_notification_id, &recv_notification_val));
	  assert(recv_notification_val == send_notification_value);
	  tot_recv_count--;
	  int curr_pos = 0;
	  for (int j = 1; j <= (nyc-2); j++)
	    vectorTmp[nxc-1][j][1] = *(segment_ptr_send + xOffset + curr_pos++);
	}

	if(vct->getYleft_neighbor() != MPI_PROC_NULL && vct->getYleft_neighbor() != myrank)
	{
	  SUCCESS_OR_DIE(gaspi_notify_waitsome(segment_id_recv, (gaspi_notification_id_t)3, 1, &recv_notification_id, GASPI_BLOCK));
	  SUCCESS_OR_DIE(gaspi_notify_reset(segment_id_recv, recv_notification_id, &recv_notification_val));
	  assert(recv_notification_val == send_notification_value);
	  tot_recv_count--;
	  int curr_pos = 0;
	  for (int i = 1; i <= (nxc-2); i++)
	    vectorTmp[i][0][1] = *(segment_ptr_send + xOffset*2 + curr_pos++);
	}

	if(vct->getYright_neighbor() != MPI_PROC_NULL && vct->getYright_neighbor() != myrank)
	{
	  SUCCESS_OR_DIE(gaspi_notify_waitsome(segment_id_recv, (gaspi_notification_id_t)4, 1, &recv_notification_id, GASPI_BLOCK));
	  SUCCESS_OR_DIE(gaspi_notify_reset(segment_id_recv, recv_notification_id, &recv_notification_val));
	  assert(recv_notification_val == send_notification_value);
	  tot_recv_count--;
	  int curr_pos = 0;
	  for (int i = 1; i <= (nxc-2); i++)
	    vectorTmp[i][nyc-1][1] = *(segment_ptr_send + xOffset*2 + yOffset + curr_pos++);
	}
	
	if(vct->getXleftYleft_neighbor() != MPI_PROC_NULL && vct->getXleftYleft_neighbor() != myrank)
	{
	  SUCCESS_OR_DIE(gaspi_notify_waitsome(segment_id_recv, (gaspi_notification_id_t)5, 1, &recv_notification_id, GASPI_BLOCK));
	  SUCCESS_OR_DIE(gaspi_notify_reset(segment_id_recv, recv_notification_id, &recv_notification_val));
	  assert(recv_notification_val == send_notification_value);
	  tot_recv_count--;
	  vectorTmp[0][0][1] = *(segment_ptr_send + xOffset * 2 + yOffset * 2);
	}
	
	if(vct->getXleftYright_neighbor() != MPI_PROC_NULL  && vct->getXleftYright_neighbor() != myrank)
	{
	  SUCCESS_OR_DIE(gaspi_notify_waitsome(segment_id_recv, (gaspi_notification_id_t)6, 1, &recv_notification_id, GASPI_BLOCK));
	  SUCCESS_OR_DIE(gaspi_notify_reset(segment_id_recv, recv_notification_id, &recv_notification_val));
	  assert(recv_notification_val == send_notification_value);
	  tot_recv_count--;
	  vectorTmp[0][nyc-1][1] = *(segment_ptr_send + xOffset * 2 + yOffset * 2 + 1);
	}
            
	if(vct->getXrightYleft_neighbor() != MPI_PROC_NULL && vct->getXrightYleft_neighbor() != myrank)
	{
	  SUCCESS_OR_DIE(gaspi_notify_waitsome(segment_id_recv, (gaspi_notification_id_t)7, 1, &recv_notification_id, GASPI_BLOCK));
	  SUCCESS_OR_DIE(gaspi_notify_reset(segment_id_recv, recv_notification_id, &recv_notification_val));
	  assert(recv_notification_val == send_notification_value);
	  tot_recv_count--;
	  vectorTmp[nxc-1][0][1] = *(segment_ptr_send + xOffset * 2 + yOffset * 2 + 2);
	}
	
	if(vct->getXrightYright_neighbor() != MPI_PROC_NULL && vct->getXrightYright_neighbor() != myrank)
	{
	  SUCCESS_OR_DIE(gaspi_notify_waitsome(segment_id_recv, (gaspi_notification_id_t)8, 1, &recv_notification_id, GASPI_BLOCK));
	  SUCCESS_OR_DIE(gaspi_notify_reset(segment_id_recv, recv_notification_id, &recv_notification_val));
	  assert(recv_notification_val == send_notification_value);
	  tot_recv_count--;
	  vectorTmp[nxc-1][nyc-1][1] = *(segment_ptr_send + xOffset * 2 + yOffset * 2 + 3);
	}
      }
 
      assert(tot_recv_count == 0);
    }
