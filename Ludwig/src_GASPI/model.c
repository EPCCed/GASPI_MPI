...
int lb_init(lb_t * lb) {
  ...
  /* Allocate GASPI segments of size 2*ndata */
  gpe_alloc_segment(nlocal[X],nlocal[Y],nlocal[Z],lb->ndist*NVEL);
...
int lb_halo(lb_t * lb) {
  ...
  if (lb_order(lb) == MODEL) {
    //lb_halo_via_copy(lb);
    lb_halo_via_copy_nonblocking_start_GASPI(lb);
    lb_halo_via_copy_nonblocking_end_GASPI(lb);
...
void halo_edges(lb_t * lb) {
  ...
  MPI_Issend(&lb->hl .sendYpp[0], nsendY, MPI_DOUBLE, Ypp, tagpp, comm, &lb->hl.sendreq[4]);
  MPI_Issend(&lb->hl .sendYpn[0], nsendY, MPI_DOUBLE, Ypn, tagpn, comm, &lb->hl.sendreq[5]);
  MPI_Issend(&lb->hl .sendYnp[0], nsendY, MPI_DOUBLE, Ynp, tagnp, comm, &lb->hl.sendreq[6]);
  MPI_Issend(&lb->hl .sendYnn[0], nsendY, MPI_DOUBLE, Ynn, tagnn, comm, &lb->hl.sendreq[7]);
  ...
  MPI_Issend(&lb->hl .sendZpp[0] , nsendZ, MPI_DOUBLE, Zpp, tagpp, comm, &lb->hl.sendreq[8]);
  MPI_Issend(&lb->hl .sendZpn[0], nsendZ, MPI_DOUBLE, Zpn, tagpn, comm, &lb->hl.sendreq[9]);
  MPI_Issend(&lb->hl .sendZnp[0], nsendZ, MPI_DOUBLE, Znp, tagnp, comm, &lb->hl.sendreq[10]);
  MPI_Issend(&lb->hl .sendZnn[0], nsendZ, MPI_DOUBLE, Znn, tagnn, comm, &lb->hl.sendreq[11]);
